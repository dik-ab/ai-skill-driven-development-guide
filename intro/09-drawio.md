...
## 実例: AI に 3 リポジトリを読ませて生成した構成図

下の図は、実在するプロジェクトの infra（Terraform）、backend、front の 3 リポジトリを AI に読ませ、人が 1 本も線を引かずに生成した構成図です。公開用にプロジェクト名と関数名だけ一般化していますが、構造はそのままです。

![AI が生成したシステム構成図](images/system-architecture-example.png)

編集可能な正本は `images/system-architecture-example.drawio` です。draw.io で開いて動かせます。

### AI が読んだもの

| リポジトリ | 読んだファイル | 図に反映されたこと |
|---|---|---|
| infra | `prod/main.tf`、各モジュールの `main.tf` | VPC、サブネット、ALB、ECS、Aurora、ElastiCache、S3、Firehose、Glue、Step Functions、CodePipeline など全リソース |
| backend | ECS タスク定義、fluent-bit 設定、`lambda/*/template.yaml`、GitHub Actions | FireLens のログ分岐、Lambda 7 本と EventBridge のスケジュール、デプロイ経路 |
| front | `Dockerfile.prod`、GitHub Actions、環境変数テンプレート | Next.js の ECS 配置、ECR への push、配信バケットの参照 |

依頼文は「この 3 つのリポジトリを見て構成図を自動生成して。AWS のアイコンを使って」の 1 行です。

### 人がやったこと

- 依頼を 1 行書いた
- 描画結果を見て、線がアイコンを貫通している箇所と、ラベルの重なりを指摘した（3 回）
- 読み取りに確信が持てなかった点を README に「未確認」として残させた

AI は 1 回目から正しい図を出したわけではありません。1 回目はタイトルと凡例が重なり、長い線が Lambda を横切っていました。ただ、指摘のたびに XML を差分で直し、数分で再描画できました。手で描き直していたら 1 回目で終わっていたはずです。

### この実例で伝わること

- **正本はコード**。図は Terraform と設定ファイルから導けるので、コードが変われば図も再生成できる
- **図が diff になる**。上の `.drawio` を編集して PR に出せば、変更箇所が行単位で分かる
- **AI は「分からない」を言える**。フロントがどのバケットを触るかは特定できず、その旨を README に残した。推測で線を引かせないことが、図の信頼性を保つ
